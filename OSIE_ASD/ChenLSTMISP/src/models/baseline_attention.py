...
class predict_head(nn.Module):
    ...
    def forward(self, encoder_feature, features, subject_embedding):
        ...
        # [N, T, Subj_num]
        predicts["attention_weights"] = attention_weights.unsqueeze(1)
...
class baseline(nn.Module):
    ...
    def forward(self, images, subject):
        ...
        if self.training:
            predicts = self.training_process(images, subject)
        else:
            predicts = self.inference(images, subject)

        return predicts

    def training_process(self, images, subject):
        # img = img.unsqueeze(0)
    ...
    def inference(self, images, subject):
        # img = img.unsqueeze(0)
        batch, _, height, width = images.size()  # build a one-hot performance embedding

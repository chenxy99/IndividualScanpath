...
class OSIESearch_by_subject(Dataset):
    """
    get OSIE data
    """
...
class OSIE(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for ids in self.imgid_to_sub[img_name]:
            ...
            # normalize
            # pos_x = np.array(fixation["X"]).astype(np.float32) / self.downscale_x
            # pos_y = np.array(fixation["Y"]).astype(np.float32) /self.downscale_y
            # duration_raw = np.array(fixation["T"]).astype(np.float32)

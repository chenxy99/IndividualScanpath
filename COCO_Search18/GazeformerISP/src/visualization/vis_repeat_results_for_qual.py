...
with open("/srv/data/COCOSearch18/TP/fixations/coco_search18_fixations_TP_validation.json") as json_file:
    fixations = json.load(json_file)

with open("../../runs/COCO_Search_baseline_20epoch_rl_decay0.1_10epoch_seed10_full_version/validation_repeat50_predicts.json") as json_file:
    predict_fixations = json.load(json_file)

qualitative_result_dir = "../../runs/COCO_Search_baseline_20epoch_rl_decay0.1_10epoch_seed10_full_version/qualitative_result_repeat50_validation_for_qual"
if not os.path.exists(qualitative_result_dir):
    os.makedirs(qualitative_result_dir)

imgs_root = "/srv/data/COCOSearch18/TP/images"
...
for object_value in tqdm(list(fixations_dict_all.keys())):
    ...
    for img_name in list(fixations_dict.keys()):
        ...
        # for index in range(0):
            ...
            selected_predict_fixation_X = [_  for _ in selected_predict_fixation["X"]]
            selected_predict_fixation_Y = [_  for _ in selected_predict_fixation["Y"]]

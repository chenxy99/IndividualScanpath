...
def main():
    ...
    train_dataset = COCO_Search18(args.img_dir, args.fix_dir, args.detector_dir,
                                  ...
                                  detector_threshold=args.detector_threshold, max_length=args.max_length)
    ...
    validation_dataset = COCO_Search18_evaluation(args.img_dir, args.fix_dir, args.detector_dir,
                                                  type="valid", split="split3", transform=transform,
                                                  detector_threshold=args.detector_threshold)
            ...
            with tqdm(total=len(train_loader)) as pbar:
                for i_batch, batch in enumerate(train_loader):
                    tmp = [batch["images"], batch["scanpaths"], batch["durations"],
                           batch["action_masks"], batch["duration_masks"], batch["attention_maps"], batch["subjects"], batch["taskints"]]
                    tmp = [_ if not torch.is_tensor(_) else _.cuda() for _ in tmp]
                    tmp = [_.view(-1, *_.shape[2:]) for _ in tmp]
                    images, scanpaths, durations, action_masks, duration_masks, attention_maps, subjects, taskints = tmp

                    optimizer.zero_grad()

                    predicts = model(images, subjects, attention_maps, taskints)
        ...
        # reinforcement learning stage
            ...
            with tqdm(total=len(train_rl_loader)) as pbar:
                for i_batch, batch in enumerate(train_rl_loader):
                    ...
                    tmp = [batch["images"], batch["subjects"], batch["fix_vectors"], batch["attention_maps"], batch["taskints"]]
                    tmp = [_ if not torch.is_tensor(_) else _.cuda() for _ in tmp]
                    # merge the first two dim
                    tmp = [_.view(-1, *_.shape[2:]) if torch.is_tensor(_) else _ for _ in tmp]
                    images, subjects, tmp_gt_fix_vectors, attention_maps, taskints = tmp
                    ...
                    while True:
                        ...
                        durations = torch.clip(durations, 0, 3)
    ...
    def validation(iteration):
        ...
        cur_metrics, cur_metrics_std, _ = comprehensive_evaluation_by_subject(all_gt_fix_vectors,
                                                                              all_predict_fix_vectors, args)
